...
class PosController extends Controller
{
    ...
    public function create()
    {
        ...
        //statuts  !annulée
        $statuts        = array_filter(Order::$statuts, function ($key) {
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $globalDiscountAmount = $globalDiscountType === 'percent'
                ? $subtotal * min($globalDiscountVal, 100) / 100
                : $globalDiscountVal;
            ...
            if ($isLivree && round($acompte, 2) !== round($total, 2)) {
                DB::rollBack();
                return redirect()->back()
                    ->with('error', "Pour une commande livrée, l'acompte (" . number_format($acompte, 0, ',', ' ') . " FCFA) doit être égal au total (" . number_format($total, 0, ',', ' ') . " FCFA).")
                    ->withInput();
            }
            ...
            foreach ($productsData as $d) {
                ...
                foreach ($d['product']->productBases as $base) {
                    ...
                    if ($quantityConsumed > 0) {
                        ...
                        // Décrémenter le stock du product_base
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            // ── Resync pivot produits + nouveaux snapshots ────────────────────────────
            $syncData = [];
            foreach ($productsData as $d) {
                $syncData[$d['product']->id] = [
                    'quantity'          => $d['qty'],
