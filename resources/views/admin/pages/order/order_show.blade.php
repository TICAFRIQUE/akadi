    ...
    <div class="section-body">
                        ...
                        <div class="dropdown-menu dropdown-menu-right">
                            @foreach ($statuts as $stKey => $st)
                                @if ($stKey === $orders->status)
                                    <span class="dropdown-item text-success font-weight-bold"
                                        style="cursor:default;background:#f0fff4">
                                        <i class="fas fa-check-circle mr-1"></i>{{ $st['label'] }}
                                    </span>
                                @elseif($stKey === 'annulée')
                                    <a href="#" data-id="{{ $orders->id }}" data-toggle="modal"
                                        data-target="#modalMotifAnnulation" class="dropdown-item text-danger btnCancel">
                                        <i class="fas fa-ban mr-1"></i>{{ $st['label'] }}
                                    </a>
                                @else
                                    <a href="{{ route('order.changeState') }}?cs={{ $stKey }}&id={{ $orders->id }}"
                                        class="dropdown-item btn-change-status-top" data-status="{{ $stKey }}">
                                        {{ $st['label'] }}
                                    </a>
                                @endif
                            @endforeach
        ...
        <div class="row">
            ...
            <div class="col-12 col-lg-8">
                    ...
                    <div class="d-flex justify-content-between align-items-center flex-wrap" style="gap:8px">
                        <div>
                            ...
                            <small class="text-muted">
                                ...
                                @if ($orders->createdBy)
                                    <br>Mise en <span
                                        class="text-{{ $statusColors[$orders->status] ?? 'secondary' }}">{{ $orders->status }}</span>
                ...
                <div class="info-section">
                    ...
                    @if ($orders->delivery_price)
                        <div class="info-row"><span>Frais
                                livraison</span><span>{{ number_format($orders->delivery_price, 0, '', ' ') }} FCFA</span>
                        </div>
                    @endif
                    @if ($orders->address)
                    ...
                    @if ($orders->delivery_date)
                        <div class="info-row"><span>Date
                                effective</span><span>{{ \Carbon\Carbon::parse($orders->delivery_date)->format('d/m/Y H:i') }}</span>
                        </div>
                    @endif
                </div>
            ...
            <div class="col-12 col-lg-4">
                ...
                {{-- Client --}}
                <div class="info-section">
                    <h6><i class="fas fa-user mr-1"></i>Client <span><a
                                href="{{ route('client.detail', $orders->user->id) }}"
                                class="btn btn-sm btn-outline-primary text-end" target="blank">Voir le profil</a></span></h6>
                    ...
                    <div class="total-recap">
                        ...
                        @if (($orders->discount ?? 0) > 0)
                            ...
                            <div class="line text-warning"><span>Remise</span><span>–
                                    {{ number_format($orders->discount, 0, '', ' ') }} {{ $globalTypeDisc === 'percent' ? '%' : 'FCFA' }}</span>
                            </div>
                        @endif
                        @if (($orders->delivery_price ?? 0) > 0)
                            <div class="line">
                                <span>Livraison</span><span>{{ number_format($orders->delivery_price, 0, '', ' ') }}
                                    FCFA</span></div>
                        @endif
                        <div class="line final"><span>Total</span><span
                                class="text-dark">{{ number_format($orders->total ?? 0, 0, '', ' ') }} FCFA</span></div>
                        <div class="line acompte"><span>Acompte
                                versé</span><span>{{ number_format($orders->acompte ?? 0, 0, '', ' ') }} FCFA</span></div>
                        <div class="line solde"><span>Solde
                                restant</span><strong>{{ number_format($orders->solde_restant ?? 0, 0, '', ' ') }}
                                FCFA</strong></div>
                    </div>
                </div>
                        ...
                        <form id="form-acompte">
                            ...
                            <div class="input-group">
                                <input type="number" id="montant-acompte" class="form-control"
                                    placeholder="Montant (FCFA)" min="1" max="{{ $orders->solde_restant }}"
                                    step="any">
                                <div class="input-group-append">
                                    <button type="submit" class="btn btn-success">Valider</button>
                                </div>
                            </div>
                            <small class="text-muted">Solde restant :
                                {{ number_format($orders->solde_restant, 0, '', ' ') }} FCFA</small>
                        </form>
                    </div>
                @endif
                ...
                @if (!in_array($orders->status, ['livrée', 'annulée']))
                    <div class="info-section">
                        ...
                        <div class="d-flex flex-column" style="gap:6px">
                            @foreach ($statuts as $stKey => $st)
                                @if ($stKey === $orders->status)
                                    <span class="btn btn-sm btn-success font-weight-bold"
                                        style="cursor:default;opacity:1">
                                        <i class="fas fa-check-circle mr-1"></i>{{ $st['label'] }}
                                    </span>
                                @elseif($stKey === 'annul\u00e9e')
                                    <a href="#" data-id="{{ $orders->id }}" data-toggle="modal"
                                        data-target="#modalMotifAnnulation"
...
@section('script')
    <script>
        ...
        function validateStatusChange(e, newStatus) {
            ...
            // // Activer si on veut forcer un acompte > 0 
            // if (ORDER_ACOMPTE <= 0) {
            //     e.preventDefault();
            //     Swal.fire({
            //         icon: 'warning',
