...
<script>
    ...
    function addPlatRow(id = '', nom = '', description = '', prix = '', disponible = true, platId = '') {
    ...
    // avant de créer les lignes, pour agir après ce script global.
    window.addEventListener('load', () => {
        @if(old('plats'))
            const oldPlats = @json(old('plats'));
            oldPlats.forEach(p => addPlatRow(p.id ?? '', p.nom ?? '', p.description ?? '', p.prix ?? '', true, p.plat_id ?? ''));
        @else
            const existingPlats = @json($existingPlats);
            existingPlats.forEach(p => addPlatRow(p.id, p.nom, p.description ?? '', p.prix, p.disponible, p.plat_id ?? ''));
